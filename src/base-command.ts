...
export default abstract class BaseCommand extends Command {
	...
	static flags = {
		...
		order: flags.enum({
			description: 'The sort order if sort parameter is provided. Default: desc',
			options: ['asc', 'desc']
		})
	};
	...
	async catch(err: Error) {
		if (err.name === 'StatusCodeError') {
			...
			(sce.error.errors || []).forEach((_err: Error) => lines.push(_err.message));
	...
	async run() {
		...
		Object.entries(options).forEach(([k, v]) => {
			if (k.startsWith('current-')) {
				...
				qs.push(`${trimmedKey}:${gitUserName.getUser()}`);
			} else {
				qs.push(`${k}:${v}`);
			}
		});
