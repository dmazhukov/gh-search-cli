...
`
	...
	static flags = {
		author: flags.string({
			description: 'Matches commits authored by a user (based on email settings).'
		}),
		committer: flags.string({
			description: 'Matches commits committed by a user (based on email settings).'
		}),
		...
		merge: flags.boolean({
			allowNo: true,
			description: '--merge filters to merge commits, --no-merge filters out merge commits.'
		}),
		hash: flags.string({
