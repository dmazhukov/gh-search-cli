...
describe('hooks', () => {
  ...
  describe('bad login', () => {
    ...
    beforeEach(() => {
      ...
      sandbox.stub(questions, 'ghe').resolves(false)
      sandbox.stub(questions, 'username').resolves('testuser')
      sandbox.stub(questions, 'password')
        .onFirstCall().resolves('wrong')
        .onSecondCall().resolves('correct')
      nock('https://api.github.com', {
        reqheaders: {
          Authorization: auth => Buffer.from(auth.split(' ')[1], 'base64').toString().endsWith(':wrong')
        }
      })
        .post('/authorizations')
        .reply(401, {})
      ...
      nock('https://api.github.com', {
        reqheaders: {
          Authorization: auth => Buffer.from(auth.split(' ')[1], 'base64').toString().endsWith(':correct')
        }
      })
        .post('/authorizations')
  ...
  describe('needs configured (ghe, no 2fa)', () => {
    ...
    beforeEach(() => {
      ...
      sandbox.stub(questions, 'apiUrl').resolves('https://github.evilcorp.com/api/v3')
      sandbox.stub(questions, 'username').resolves('testuser')
      sandbox.stub(questions, 'password').resolves('password')
