...
describe('ghs commands', () => {
	...
	beforeEach(() => {
		...
		sandbox.stub(_paginator, 'next').resolves(true);
	...
	describe('repositories', () => {
		...
		describe('--current-x flags', () => {
			test
				.stub(_git, 'getUser', sinon.stub().returns('feinoujc'))
				.stdout()
				.command([...args, '--current-user'])
				.it('includes the current git user', ctx => {
					expect(_git.getUser()).to.be.equal('feinoujc');
					expect(ctx.stdout).to.be.contain('feinoujc');
				});
		});
	});
